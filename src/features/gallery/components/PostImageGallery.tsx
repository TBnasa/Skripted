...
export function PostImageGallery({
  ...
  return (
    <div className="lg:col-span-5 space-y-10 animate-slide-up" style={{ animationDelay: '0.2s' }}>
      <div className="bg-white/[0.01] border border-[var(--color-border)] rounded-3xl p-8 md:p-10 shadow-2xl relative overflow-hidden group">
        ...
        <div className="flex items-center justify-between mb-6">
          ...
          <button 
            ...
            className="flex items-center gap-2 px-3 py-1 bg-white/5 hover:bg-white/10 rounded-lg text-[10px] font-bold text-[var(--color-text-muted)] hover:text-[var(--color-text-primary)] transition-all border border-[var(--color-border)]"
          >
        ...
        {isEditingPost ? (
          <textarea 
            ...
            className="w-full bg-white/5 border border-[var(--color-border-hover)] rounded-xl p-4 text-[var(--color-text-primary)] focus:outline-none focus:border-[var(--color-border-active)] min-h-[150px] resize-none"
          />
        ) : (
           <div className="prose prose-invert prose-zinc text-[var(--color-text-secondary)] whitespace-pre-wrap leading-relaxed max-w-none text-base italic">
        ...
        {post.tags && post.tags.length > 0 && (
          <div className="flex flex-wrap gap-2 mt-8">
            {post.tags.map((tag: string) => (
              <span key={tag} className="px-3 py-1 bg-white/[0.03] border border-[var(--color-border)] rounded-lg text-[9px] font-bold text-[var(--color-text-muted)] uppercase tracking-widest flex items-center gap-1.5">
      ...
      {post.image_urls && post.image_urls.length > 0 && (
        <div className="bg-black/20 border border-[var(--color-border)] rounded-3xl overflow-hidden shadow-2xl ring-1 ring-white/5">
          ...
          {post.image_urls.length > 1 && (
            <div className="p-5 bg-white/[0.01] border-t border-[var(--color-border)] flex gap-3 overflow-x-auto custom-scrollbar">
              {post.image_urls.map((url: string, idx: number) => (
                <button 
                  key={idx}
                  onClick={() => setActiveImage(idx)}
                  className={`relative w-24 h-16 shrink-0 rounded-xl overflow-hidden border-2 transition-all ${
                    activeImage === idx ? 'border-[var(--color-border-active)] scale-105 shadow-lg' : 'border-transparent opacity-40 hover:opacity-100'
                  }`}
                >
