    ...
    <nav className="fixed top-0 left-0 right-0 z-50 border-b border-[var(--color-border)] bg-[var(--color-bg-primary)]/80 backdrop-blur-2xl">
      <div className="mx-auto flex h-16 max-w-7xl items-center justify-between px-6">
        ...
        <Link href="/" className="flex items-center gap-3 group">
          <div className="relative flex h-8 w-8 items-center justify-center rounded-lg bg-[#10b981] transition-all duration-300 group-hover:bg-[#059669]">
          ...
          <span className="text-sm font-bold tracking-tight text-white">
            Skripted{isChatPage && <span className="text-emerald-500/80 ml-1">Engine</span>}
          </span>
        </Link>
        ...
        <div className="flex items-center gap-2 sm:gap-4">
          {/* Language Switcher */}
          <div className="flex items-center gap-1 bg-white/[0.03] p-1 rounded-lg border border-white/[0.05] mr-2">
            <button
              onClick={() => switchLanguage('en')}
              className={`px-2 py-1 text-[9px] font-bold rounded-md transition-all ${
                lang === 'en' || lang.startsWith('en') ? 'bg-zinc-800 text-white' : 'text-zinc-600 hover:text-zinc-400'
              }`}
            >
              EN
            </button>
            <button
              ...
              className={`px-2 py-1 text-[9px] font-bold rounded-md transition-all ${
                lang === 'tr' || lang.startsWith('tr') ? 'bg-zinc-800 text-white' : 'text-zinc-600 hover:text-zinc-400'
              }`}
            >
              TR
            </button>
          ...
          <div className="hidden md:flex items-center gap-4">
            <a
              ...
              className="px-3 py-2 text-[11px] font-bold uppercase tracking-wider text-zinc-500 transition-all duration-300 hover:text-white rounded-lg"
            >
              GitHub
            </a>
            ...
            <Link
              href="/pricing"
              className="flex items-center gap-2 px-3 py-2 text-[11px] font-bold uppercase tracking-wider text-zinc-500 transition-all duration-300 hover:text-emerald-500 rounded-lg relative group"
            >
              {t('pricing.title')}
              <span className="ml-1 px-1.5 py-0.5 text-[8px] font-black bg-emerald-500/10 text-emerald-500 border border-emerald-500/20 rounded uppercase">
                New
              </span>
            </Link>
            ...
            {pathname !== '/chat' && (
              <>
                <Link
                  href="/dashboard"
                  className="px-3 py-2 text-[11px] font-bold uppercase tracking-wider text-zinc-500 hover:text-white transition-all"
                >
                  {t('general.dashboard')}
                </Link>
                <Link
                  href="/chat"
                  className="btn-premium btn-primary text-[10px] font-bold px-5"
                >
