 ...
 * 
 * Uses NVIDIA Nemotron-3 Super (120B) to verify user feedback
 * and ensure high-quality RAG data.
 */
...
export class JudgeService {
  ...
  static async analyzeFeedback(
    ...
    const systemPrompt = `You are the 'Skripted AI Judge'. Your job is to verify user feedback for validity.
Context:
...
- Output EXACTLY in this JSON format:
{
  ...
  "isVerified": true/false
    ...
    try {
      ...
      if (!rawVerdict) throw new Error("Empty AI response");

      const verdict = JSON.parse(rawVerdict);
      
      return {
        trustScore: Math.min(100, Math.max(0, verdict.trustScore)),
        analysis: verdict.analysis || "Analiz yapılamadı.",
        isVerified: !!verdict.isVerified
      };
    } catch (error) {
      console.error('[Judge] Analysis failed:', error);
      return { trustScore: 0, analysis: `Hata: ${String(error)}`, isVerified: false };
    }
  }
}
